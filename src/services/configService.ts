 ...
 */
export async function fetchColorPaletteConfig(forceRefresh: boolean = false): Promise<{
  config: FileSuffixColorConfig;
  source: 'remote' | 'local';
  lastUpdated?: string;
  error?: string;
  ...
  try {
    ...
    const url = forceRefresh ? `${API_CONFIG_URL}?refresh=true` : API_CONFIG_URL;
    ...
    const { config, source, lastUpdated, error: apiError, cached: serverCached } = apiResponse;
    ...
    console.log(`Successfully loaded config from API (${source}, server-cached: ${serverCached}):`, config.version);
    ...
    console.warn('Failed to fetch config from API, falling back to local:', error);
...
export function getColorPaletteConfigSync(): {
  ...
  try {
    const cached = getCachedConfig();
    if (cached && isConfigCacheValid(cached) && cached.config && cached.config.suffixConfigs) {
      return {
        config: cached.config,
        source: 'cached'
      };
  ...
  if (!config || !config.suffixConfigs || typeof config.suffixConfigs !== 'object') {
    ...
    throw new Error(
      ...
      'Config structure: ' + JSON.stringify(config ? Object.keys(config) : 'null')
    );
  }
