...
import { getColorPaletteConfigSync, fetchColorPaletteConfig } from '@/services/configService';
...
export interface ColorLayerConfig {
  ...
  customRender?: (ctx: CanvasRenderingContext2D, bounds: { x: number; y: number; width: number; height: number }, scale: number) => void;
}
...
export function createFileColorHighlightLayers(
  ...
  console.log('[FileColorMapping] Config received:', {
    ...
    suffixConfigsType: config?.suffixConfigs ? typeof config.suffixConfigs : 'undefined',
    suffixConfigsKeys: config?.suffixConfigs ? Object.keys(config.suffixConfigs).slice(0, 5) : 'undefined',
  });
  ...
  sortedSuffixes.forEach(([suffix, files]) => {
    ...
    const primaryLayer: HighlightLayer = {
      ...
      items: files.map((path) => ({
        ...
        ...(suffixConfig.primary.coverOptions && { coverOptions: suffixConfig.primary.coverOptions }),
        ...(suffixConfig.primary.customRender && { customRender: suffixConfig.primary.customRender }),
      })),
    };
  ...
  if (includeUnmatched && unmatchedFiles.length > 0 && defaultConfig) {
    const defaultLayer: HighlightLayer = {
      ...
      items: unmatchedFiles.map((path) => ({
        ...
        ...(defaultConfig.primary.coverOptions && { coverOptions: defaultConfig.primary.coverOptions }),
        ...(defaultConfig.primary.customRender && { customRender: defaultConfig.primary.customRender }),
      })),
    };
...
export function getFileColorConfig(): FileSuffixColorConfig {
  ...
  console.log('[FileColorMapping] Config loaded:', {
    ...
    suffixConfigCount: result.config?.suffixConfigs ? Object.keys(result.config.suffixConfigs).length : 0,
...
export const DEFAULT_FILE_COLORS: Record<string, string> = Object.entries(DEFAULT_FILE_CONFIGS.suffixConfigs).reduce(
